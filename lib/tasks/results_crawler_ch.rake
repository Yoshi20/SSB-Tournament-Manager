...
namespace :results_crawler_ch do
  foundTournaments = []
  notFoundPlayers = []
  ...
  task all: :environment do
    Rake::Task["results_crawler_ch:createTournaments"].invoke
    Rake::Task["results_crawler_ch:findPlayers"].invoke
    Rake::Task["results_crawler_ch:createMatches"].invoke
    Rake::Task["results_crawler_ch:createResults"].invoke
    puts "\ndone"
  ...
  task createTournaments: :environment do
    links = [
      ...
      #'https://braacket.com/league/SSBUCHPRs/tournament?rows=200&page=2',
      'https://braacket.com/league/weekrive/tournament?rows=200',
    ]
    links.each do |link|
      puts "\nCrawling #{link}..."
  ...
  task findPlayers: :environment do
    allGamerTags = Player.all_from('ch').map {|p| p.gamer_tag}
    allGamerTags += AlternativeGamerTag.all_from('ch').map {|p| p.gamer_tag}
      ...
      doc.css('table tbody tr').each do |tr|
        ...
        if tr.css('td a')[1].nil?
          ...
          if firstGamerTag != gamerTag
            ...
            AlternativeGamerTag.create(player_id: player.id, gamer_tag: firstGamerTag, country_code: 'ch') unless player.nil?
          end
        end
