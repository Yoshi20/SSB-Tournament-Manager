...
namespace :results_crawler_us_ca do
  foundTournaments = []
  notFoundPlayers = []
  ...
  task all: :environment do
    Rake::Task["results_crawler_us_ca:createTournaments"].invoke
    Rake::Task["results_crawler_us_ca:findPlayers"].invoke
    Rake::Task["results_crawler_us_ca:createMatches"].invoke
    Rake::Task["results_crawler_us_ca:createResults"].invoke
    puts "\ndone"
  ...
  task createTournaments: :environment do
    links = [
      'https://braacket.com/league/SSBUUSCAPRs/tournament?rows=200',
    ]
    links.each do |link|
      puts "\nCrawling #{link}..."
  ...
  task findPlayers: :environment do
    allGamerTags = Player.all_from('us_ca').map {|p| p.gamer_tag}
    allGamerTags += AlternativeGamerTag.all_from('us_ca').map {|p| p.gamer_tag}
      ...
      doc.css('table tbody tr').each do |tr|
        ...
        if tr.css('td a')[1].nil?
          ...
          if firstGamerTag != gamerTag
            ...
            AlternativeGamerTag.create(player_id: player.id, gamer_tag: firstGamerTag, country_code: 'us_ca') unless player.nil?
          end
        end
