...
namespace :results_crawler_pt do
  foundTournaments = []
  notFoundPlayers = []
  ...
  task all: :environment do
    Rake::Task["results_crawler_pt:createTournaments"].invoke
    Rake::Task["results_crawler_pt:findPlayers"].invoke
    Rake::Task["results_crawler_pt:createMatches"].invoke
    Rake::Task["results_crawler_pt:createResults"].invoke
    puts "\ndone"
  ...
  task findPlayers: :environment do
    Time.use_zone("London") {
      allGamerTags = Player.all_from('pt').map {|p| p.gamer_tag}
      allGamerTags += AlternativeGamerTag.all_from('pt').map {|p| p.gamer_tag}
        ...
        doc.css('table tbody tr').each do |tr|
          ...
          if tr.css('td a')[1].nil?
            ...
            if firstGamerTag != gamerTag
              ...
              AlternativeGamerTag.create(player_id: player.id, gamer_tag: firstGamerTag, country_code: 'pt') unless player.nil?
            end
          end
