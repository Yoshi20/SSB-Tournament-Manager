...
namespace :tournaments_crawler_it do
  desc "Calls all the tournaments web crawler tasks"
  task all: :environment do
    tCtr = Tournament.all_from('it').count
    puts "Running all tournaments web crawlers..."
    Rake::Task["tournaments_crawler_it:braacket"].invoke
    Rake::Task["tournaments_crawler_it:smash_gg"].invoke
    puts "\ndone -> #{Tournament.all_from('it').count - tCtr} new tournament(s)\n"
  end
  ...
  task braacket: :environment do
    ...
    doc = Nokogiri::HTML(URI.open('https://braacket.com/tournament/search?rows=100&country=it&game=ssbu&status=1'))
  ...
  task smash_gg: :environment do
    ...
    # doc = Nokogiri::HTML(URI.open('https://smash.gg/api/-/gg_api./public/tournaments/schedule?filter={%22upcoming%22%3Atrue%2C%22videogameIds%22%3A%221386%22%2C%22countryCode%22%3A%22IT%22}&page=1&per_page=100&returnMeta=true'))
    doc = URI.open('https://smash.gg/api/-/gg_api./public/tournaments/schedule?filter={%22upcoming%22%3Atrue%2C%22videogameIds%22%3A%221386%22%2C%22countryCode%22%3A%22IT%22}&page=1&per_page=100&returnMeta=true').read
