...
namespace :results_crawler_is do
  foundTournaments = []
  notFoundPlayers = []
  ...
  task all: :environment do
    Rake::Task["results_crawler_is:createTournaments"].invoke
    Rake::Task["results_crawler_is:findPlayers"].invoke
    Rake::Task["results_crawler_is:createMatches"].invoke
    Rake::Task["results_crawler_is:createResults"].invoke
    puts "\ndone"
  ...
  task createTournaments: :environment do
    links = [
      'https://braacket.com/league/SSBUISPRs/tournament?rows=200',
    ]
    links.each do |link|
      puts "\nCrawling #{link}..."
  ...
  task findPlayers: :environment do
    allGamerTags = Player.all_from('is').map {|p| p.gamer_tag}
    allGamerTags += AlternativeGamerTag.all_from('is').map {|p| p.gamer_tag}
      ...
      doc.css('table tbody tr').each do |tr|
        ...
        if tr.css('td a')[1].nil?
          ...
          if firstGamerTag != gamerTag
            ...
            AlternativeGamerTag.create(player_id: player.id, gamer_tag: firstGamerTag, country_code: 'is') unless player.nil?
          end
        end
