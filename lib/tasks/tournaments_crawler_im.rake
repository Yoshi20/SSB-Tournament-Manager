...
namespace :tournaments_crawler_im do
  desc "Calls all the tournaments web crawler tasks"
  task all: :environment do
    tCtr = Tournament.all_from('uk').count
    puts "Running all tournaments web crawlers..."
    Rake::Task["tournaments_crawler_im:braacket"].invoke
    Rake::Task["tournaments_crawler_im:smash_gg"].invoke
    puts "\ndone -> #{Tournament.all_from('uk').count - tCtr} new tournament(s)\n"
  end
  ...
  task braacket: :environment do
    Time.use_zone("London") {
      ...
      doc = Nokogiri::HTML(URI.open('https://braacket.com/tournament/search?rows=100&country=im&game=ssbu&status=1'))
      doc.css('div.my-panel-mosaic').each_with_index do |p, i|
        ...
        externalTournament.region = 'north_west'
  ...
  task smash_gg: :environment do
    Time.use_zone("London") {
      ...
      # doc = Nokogiri::HTML(URI.open('https://smash.gg/api/-/gg_api./public/tournaments/schedule?filter={%22upcoming%22%3Atrue%2C%22videogameIds%22%3A%221386%22%2C%22countryCode%22%3A%22IM%22}&page=1&per_page=100&returnMeta=true'))
      doc = URI.open('https://smash.gg/api/-/gg_api./public/tournaments/schedule?filter={%22upcoming%22%3Atrue%2C%22videogameIds%22%3A%221386%22%2C%22countryCode%22%3A%22IM%22}&page=1&per_page=100&returnMeta=true').read
