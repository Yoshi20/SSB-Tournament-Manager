...
module Thredded
  class ApplicationController < ::ApplicationController # rubocop:disable Metrics/ClassLength
    ...
    def thredded_moderator?
      ...
      @is_thredded_moderator = !thredded_current_user.thredded_can_moderate_messageboards.where(country_code: session['country_code']).empty?
    end
    ...
    def topics_scope
      @topics_scope ||=
        if messageboard_or_nil
          ...
          policy_scope(Thredded::Topic.all).joins(:messageboard).merge(policy_scope(Thredded::Messageboard.where(country_code: session['country_code'])))
        end
    end
