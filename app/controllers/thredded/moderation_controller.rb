...
module Thredded
  class ModerationController < Thredded::ApplicationController
    ...
    def users
      @users = Thredded.user_class.where(country_code: session['country_code'])
    ...
    def user
      ...
      posts_scope = @user.thredded_posts.where(country_code: session['country_code'])
        .where(messageboard_id: policy_scope(Messageboard.where(country_code: session['country_code'])).pluck(:id))
        .order_newest_first
        .includes(:postable)
        .send(Kaminari.config.page_method_name, current_page)
    ...
    def moderatable_posts
      if moderatable_messageboards == Thredded::Messageboard.where(country_code: session['country_code'])
        Thredded::Post.where(country_code: session['country_code'])
      else
        Thredded::Post.where(country_code: session['country_code']).where(messageboard_id: moderatable_messageboards)
      end
    end
    ...
    def accessible_post_moderation_records
      ...
      # end
      moderatable_messageboards == Thredded::Messageboard.where(country_code: session['country_code'])
      Thredded::PostModerationRecord.where(messageboard_id: moderatable_messageboards)
    end

    def moderatable_messageboards
      @moderatable_messageboards ||= thredded_current_user.thredded_can_moderate_messageboards.where(country_code: session['country_code'])
    end
