...
module Thredded
  class MessageboardGroupsController < Thredded::ApplicationController
    ...
    def create
      ...
      authorize @messageboard_group, :create?
      @messageboard_group.country_code = session['country_code']
    ...
    def show
      @group = Thredded::MessageboardGroup.where(country_code: session['country_code']).where(id: params[:id])
      @groups = Thredded::MessageboardGroupView.grouped(
        policy_scope(Thredded::Messageboard.where(country_code: session['country_code']).where(group: params[:id])),
