...
module Thredded
  class ThemePreviewsController < Thredded::ApplicationController
    def show # rubocop:disable Metrics/MethodLength
      @messageboard = Thredded::Messageboard.where(country_code: session['country_code']).first
      ...
      @messageboard_views = [
        ...
        Thredded::MessageboardView.new(Thredded::Messageboard.where(country_code: session['country_code']).first(2)[-1], unread_topics_count: 2),
        Thredded::MessageboardView.new(Thredded::Messageboard.where(country_code: session['country_code']).first(3)[-1]),
      ]
